...
try:
    ...
    # (Secrets dosyasında [supabase] altında mı yoksa düz mü olduğuna bakar)
    
    if "supabase" in st.secrets:
        # [supabase] başlığı altındaysa
...
def fetch_bis_cbpol_tr(start_date, end_date):
    """
    BIS üzerinden Politika Faizi (User's request: Don't remove BIS)
    """
    try:
