...
def require_login_page():
    """
    pages/*.py dosyalarının en üstünde çağrılır. Eğer giriş yapılmamışsa
    sidebar'ı gizler, giriş uyarısı gösterir ve sayfayı durdurur.
    """
    if not check_login():
        st.markdown(
            """
...
# ---------------------------------------------------------------------------
# UI Teması (eski theme.py içeriği — her sayfa zaten utils import ettiği için
# ayrı dosyaya gerek yok; Streamlit Cloud'da pages/ import yolu sorunu çıkmaz)
# ---------------------------------------------------------------------------
def apply_theme():
    st.markdown(
        """
