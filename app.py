...
def send_verification_email(code, action_desc="Veri Silme"):
    ...
    try:
        msg = MIMEText(f"Doğrulama Kodu: {code}")
...
if not st.session_state['giris_yapildi']:
    ...
    with c2:
        st.markdown("""<div class="login-container"><h1 class="login-header">📊 Finansal Tahmin Terminali v5</h1><p style="color: #666; margin-bottom: 20px;">Lütfen erişim için şifrenizi giriniz.</p></div>""", unsafe_allow_html=True)
...
if page == "Dashboard (Analiz)":
    ...
    if df_t.empty:
        ...
        with st.sidebar:
            ...
            selected_users = st.multiselect("Karşılaştırılacak Katılımcılar", all_users)
        ...
        def plot_forecast_vs_realized(metric_label, forecast_col, realized_col, title):
            ...
            if selected_users:
                ...
                for user in selected_users:
                    ...
                    if not ud.empty:
                        fig.add_trace(go.Scatter(x=ud['hedef_donemi'], y=ud[forecast_col], mode='lines+markers', name=user))
            # B) KULLANICI SEÇİLMEDİYSE (MEDYAN)
            else:
                forecast_agg = filtered_df.groupby("hedef_donemi")[forecast_col].median().reset_index()
                fig.add_trace(go.Scatter(x=forecast_agg['hedef_donemi'], y=forecast_agg[forecast_col], mode='lines+markers', name='Piyasa Medyanı', line=dict(color='blue', width=4), marker=dict(size=8)))
            ...
            if not realized_df.empty and realized_col in realized_df.columns:
                ...
                if not real_data.empty:
                    fig.add_trace(go.Scatter(x=real_data['Donem'], y=real_data[realized_col], mode='lines+markers', name='Gerçekleşen', line=dict(color='red', width=3, dash='dot'), marker=dict(symbol='x', size=10, color='red')))

            fig.update_layout(title=title, hovermode="x unified", legend=dict(orientation="h", y=1.1))
            return fig

        g1, g2 = st.columns(2)
        with g1: st.plotly_chart(plot_forecast_vs_realized("PPK", "tahmin_ppk_faiz", "PPK Faizi", "PPK Beklentisi vs Gerçekleşen"), use_container_width=True)
        with g2: st.plotly_chart(plot_forecast_vs_realized("YS PPK", "tahmin_yilsonu_faiz", "PPK Faizi", "Yıl Sonu Faiz Beklentisi"), use_container_width=True)

        g3, g4 = st.columns(2)
        with g3: st.plotly_chart(plot_forecast_vs_realized("Aylık Enf", "tahmin_aylik_enf", "Aylık TÜFE", "Aylık TÜFE Beklentisi"), use_container_width=True)
        with g4: st.plotly_chart(plot_forecast_vs_realized("Yıllık Enf", "tahmin_yillik_enf", "Yıllık TÜFE", "Yıllık TÜFE Beklentisi"), use_container_width=True)
        ...
        if realized_df.empty:
            ...
            calculate_best("tahmin_yilsonu_enf", "Yıllık TÜFE", "🎯 En İyi Yıllık Enf.", perf_cols[0])
            calculate_best("tahmin_aylik_enf", "Aylık TÜFE", "🎯 En İyi Aylık Enf.", perf_cols[1])
            calculate_best("tahmin_ppk_faiz", "PPK Faizi", "🎯 En İyi PPK", perf_cols[2])
...
# ========================================================
    ...
    if not df_t.empty:
        ...
        fig = px.line(report_df.sort_values("tahmin_tarihi"), x="tahmin_tarihi", y="tahmin_yilsonu_enf", color="gorunen_isim", markers=True, title="Enflasyon Beklentilerinin Evrimi")
...
# ========================================================
    ...
    if not df_t.empty:
        ...
        with col_header: st.markdown("### Tüm Kayıtlar")
        with col_toggle: silme_modu = st.toggle("🗑️ Kayıt Silme Modunu Aç")

        if silme_modu:
            st.warning("⚠️ Silmek istediğiniz kayıtları seçin.")
            ...
            edited_df = st.data_editor(df_t[final_cols], column_config={"Sec": st.column_config.CheckboxColumn("Sil?", default=False), "tahmin_tarihi": st.column_config.DateColumn("Tarih", format="DD.MM.YYYY")}, disabled=[c for c in df_t.columns if c != "Sec"], use_container_width=True, hide_index=True, key="editor_silme")
            secilenler = edited_df[edited_df["Sec"] == True]
            if not secilenler.empty:
                st.markdown(f"🔴 **{len(secilenler)}** kayıt seçildi.")
                if st.button("🗑️ SEÇİLENLERİ KALICI OLARAK SİL", type="primary"):
                    try:
                        supabase.table(TABLE_TAHMIN).delete().in_("id", secilenler["id"].tolist()).execute()
...
# ========================================================
    ...
    if not df_t.empty:
        ...
        # PANDAS STYLE İLE BOŞLARI GRİ YAPMA
        s = pivot.style.background_gradient(cmap="RdYlGn_r", axis=None)
        s.highlight_null(props="background-color: #d3d3d3;") # GRI RENK KODU
        
        st.dataframe(s.format("{:.2f}"), use_container_width=True, height=600)
...
# ========================================================
    ...
    if uploaded_file:
        try:
            ...
            df_upload.columns = df_upload.columns.str.strip() # Sütun boşluklarını temizle
            if "Katılımcı Adı" not in df_upload.columns: st.error("Hata: 'Katılımcı Adı' sütunu bulunamadı."); st.stop()
            ...
            if st.button("🚀 Verileri Veritabanına İşle"):
                ...
                for index, row in df_upload.iterrows():
                    try:
                        user = str(row["Katılımcı Adı"]).strip(); hedef_donemi = str(row["Hedef Dönem (YYYY-AA)"]).strip(); cat = str(row.get("Kategori", "Bireysel")); link = str(row.get("Link", "")); raw_date = row["Tarih (YYYY-AA-GG)"]
                        if user and (user not in existing_users_set):
                            try: supabase.table(TABLE_KATILIMCI).insert({"ad_soyad": user, "kategori": cat}).execute(); existing_users_set.add(user)
                            except: pass
                        def cv(val): 
...
# ========================================================
    ...
    with st.form("entry_form"):
        ...
        if page == "PPK Girişi":
            ...
            data = {"tahmin_ppk_faiz": v1, "min_ppk_faiz": min1, "max_ppk_faiz": max1, "tahmin_yilsonu_faiz": v2, "min_yilsonu_faiz": min2, "max_yilsonu_faiz": max2}
        else: # Enflasyon
            c1, c2, c3 = st.columns(3)
            v1 = c1.number_input("Aylık Enflasyon (%)", step=0.01, format="%.2f"); min1 = c1.number_input("Min Aylık", step=0.01); max1 = c1.number_input("Max Aylık", step=0.01)
            v2 = c2.number_input("Yıllık Enflasyon (%)", step=0.01, format="%.2f"); min2 = c2.number_input("Min Yıllık", step=0.01); max2 = c2.number_input("Max Yıllık", step=0.01)
            v3 = c3.number_input("Yıl Sonu Enflasyon (%)", step=0.01, format="%.2f"); min3 = c3.number_input("Min YS", step=0.01); max3 = c3.number_input("Max YS", step=0.01)
            data = {"tahmin_aylik_enf": v1, "min_aylik_enf": min1, "max_aylik_enf": max1, "tahmin_yillik_enf": v2, "min_yillik_enf": min2, "max_yillik_enf": max2, "tahmin_yilsonu_enf": v3, "min_yilsonu_enf": min3, "max_yilsonu_enf": max3}
            
        kat_sayisi = st.number_input("Katılımcı Sayısı (N)", value=1)
        data["katilimci_sayisi"] = kat_sayisi
        st.markdown("---"); st.write("Opsiyonel: Gelecek Yıl İçin Ek Tahmin")
    ...
    if st.button("🔄 Veri Havuzundaki Kişileri Buraya Eşle (Sync)"):
        with st.spinner("Taranıyor..."):
            ...
            if not all_forecast_users.empty:
                ...
                res_k = supabase.table(TABLE_KATILIMCI).select("ad_soyad").execute(); existing_users = set([r['ad_soyad'] for r in res_k.data])
                added_count = 0
                for _, row in unique_forecast_users.iterrows():
                    user = row['kullanici_adi']; cat = row.get('kategori', 'Bireysel')
                    if user not in existing_users:
                        try: supabase.table(TABLE_KATILIMCI).insert({"ad_soyad": user, "kategori": cat}).execute(); added_count += 1
                        except: pass
                if added_count > 0: st.success(f"✅ {added_count} kişi eklendi!"); time.sleep(1.5); st.rerun()
                else: st.info("✅ Liste güncel.")
    st.markdown("---")
    with st.expander("➕ Manuel Yeni Kişi Ekle"):
        with st.form("new_kat"):
            ad = st.text_input("Ad / Kurum Adı"); cat = st.radio("Kategori", ["Bireysel", "Kurumsal"], horizontal=True)
            if st.form_submit_button("Ekle"):
                if ad: supabase.table(TABLE_KATILIMCI).insert({"ad_soyad": normalize_name(ad), "kategori": cat}).execute(); st.success("Eklendi!"); time.sleep(1); st.rerun()
    st.markdown("---")
    res = supabase.table(TABLE_KATILIMCI).select("*").order("ad_soyad").execute(); df_kat = pd.DataFrame(res.data)
    if not df_kat.empty:
        st.dataframe(df_kat, use_container_width=True)
        st.markdown("### 🚨 Tehlikeli Bölge")
        with st.expander("💣 Tüm Veritabanını Sıfırla (Mail Onaylı)"):
            ...
            if st.session_state['delete_stage'] == 0:
                ...
                if st.button("Sıfırlama Talebi Başlat"):
                    if admin_pwd == SITE_SIFRESI:
                        otp = str(random.randint(100000, 999999)); st.session_state['otp_code'] = otp
                        if send_verification_email(otp, "TÜM VERİTABANI SIFIRLAMA"): st.success("Mail gönderildi!"); st.session_state['delete_stage'] = 1; st.rerun()
                    else: st.error("Hatalı Şifre")
            elif st.session_state['delete_stage'] == 1:
                user_otp = st.text_input("Gelen 6 Haneli Kodu Girin")
                if st.button("🔥 ONAYLA VE SİL 🔥"):
                    if user_otp == st.session_state.get('otp_code'):
                        supabase.table(TABLE_TAHMIN).delete().neq("id", 0).execute(); supabase.table(TABLE_KATILIMCI).delete().neq("id", 0).execute()
                        st.success("Veritabanı Sıfırlandı!"); st.session_state['delete_stage'] = 0; time.sleep(2); st.rerun()
